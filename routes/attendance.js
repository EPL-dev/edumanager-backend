// routes/attendance.js — avec DELETE
...
router.get('/', allRoles, async (req, res) => {
  try {
    ...
    if (req.query.date)    { where += ' AND DATE(a.date) = ?'; params.push(req.query.date); }
...
router.post('/session', adminOnly, async (req, res) => {
  try {
    ...
    await pool.execute('DELETE FROM attendance WHERE subjectId = ? AND DATE(date) = ?', [subjectId, date]);
...
router.put('/:id', adminOnly, async (req, res) => {
  try {
    await pool.execute('UPDATE attendance SET status=? WHERE id=?', [req.body.status, req.params.id]);
